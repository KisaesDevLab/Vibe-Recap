...
4. **No outbound network calls from the worker** except to Ollama (bundled container, or the host gateway when `OLLAMA_URL` points at the host). Enforce with an egress-denied network in compose plus an explicit allow for the Ollama target. The API container may reach `licensing.kisaes.com` only. There are no other integrations; do not add any.
5. **Retention is enforced by a job, not by trust.** The purge worker runs hourly and is the only thing that deletes files. Purges are logged to the audit table.
