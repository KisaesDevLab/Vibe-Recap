...
import { BREAKGLASS_EMAIL, isBreakglass } from "../lib/vibeAuthUsers.js";
...
async function adminCount(app: FastifyInstance, excludeId?: string): Promise<number> {
  const [row] = await app.db
    .select({ n: count() })
    .from(users)
    // The break-glass account (Q57) is an emergency login, not an admin at a desk: it never
    // counts as the "other admin" that makes demoting the last real one safe.
    .where(and(eq(users.role, "admin"), eq(users.disabled, false), ne(users.email, BREAKGLASS_EMAIL), excludeId ? ne(users.id, excludeId) : undefined));
  return row?.n ?? 0;
}

const BREAKGLASS_MANAGED =
  "This is the single sign-on break-glass account. It is managed by Vibe Auth: rotate its password with the breakglass command (docs/sso.md); it cannot be disabled, demoted or reset here.";
...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.post("/api/users", { preHandler: requireRole("admin") }, async (req, reply) => {
    ...
    if (email === BREAKGLASS_EMAIL) throw conflict("That address is reserved for the single sign-on break-glass account");
  ...
  app.post("/api/users/:id/send-reset-link", { preHandler: requireRole("admin") }, async (req) => {
    ...
    if (isBreakglass(u)) throw conflict(BREAKGLASS_MANAGED);
    ...
    // An administrator chose to give this account a local password: self-service opens up too (Q57).
    if (u.ssoOnly) await app.db.update(users).set({ ssoOnly: false, updatedAt: new Date() }).where(eq(users.id, id));
    await audit(app.db, { actor: actorOf(req), action: "user.password_reset_link_sent", target: { type: "user", id }, ip: req.ip, meta: u.ssoOnly ? { ssoOnlyCleared: true } : {} });
    return { ok: true };
  });
  ...
  app.patch("/api/users/:id", { preHandler: requireRole("admin") }, async (req) => {
    ...
    const [u] = await app.db.select().from(users).where(eq(users.id, id)).limit(1);
    if (!u) throw notFound("User not found");
    // In every mode, not only oidc_only: the api refuses to start in oidc_only without this account.
    if (isBreakglass(u) && ((body.role !== undefined && body.role !== "admin") || body.disabled === true)) throw conflict(BREAKGLASS_MANAGED);
  ...
  app.post("/api/users/:id/reset-password", { preHandler: requireRole("admin") }, async (req) => {
    ...
    if (!u) throw notFound("User not found");
    if (isBreakglass(u)) throw conflict(BREAKGLASS_MANAGED);
    await app.db.update(users).set({ passwordHash: await hashPassword(body.tempPassword), mustChangePassword: true, ssoOnly: false, failedLogins: 0, lockedUntil: null, updatedAt: new Date() }).where(eq(users.id, id));
